...
public class c5_CreatingSequence {
    ...
    public void repeat() {
        ...
        Flux<Integer> repeated = Flux.generate(() -> 0,
                (state, sink) -> {
     ...
     * <p>
     * Answer:
     * - What is difference between `generate` and `create`? generate = sync (sink.next는 consumer안에서 한 번만 불린다.), create = async?
     * - What is difference between `create` and `push`? push = async + single-threaded
     */
    @Test
    public void generate_programmatically() {
        ...
        Flux<Integer> generateFlux = Flux.generate(sink -> {
            sink.next(counter.incrementAndGet());
            if (5 == counter.get()) sink.complete();
        });
     ...
     * create : multi thread 지원 O
     * push : multi thread 지원 X , 중간중간 간섭 현상도 있음.
     */
    @Test
