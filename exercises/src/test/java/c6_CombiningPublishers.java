...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void i_am_rubber_you_are_glue() {
        Flux<Integer> numbers = numberService1().concatWith(numberService2()); // concatMap을 어떻게 활용해야하지?...
        //don't change below this line
    ...
    public void mail_box_switcher() {
        Flux<Message> myMail = mailBoxPrimary().switchOnFirst((signal, messageFlux) -> {
            if (signal.get().metaData.startsWith("spam")) {
    ...
    public void cleanup() {
        ...
        Flux<String> stream = Flux.usingWhen(StreamingConnection.startStreaming(),
                                         music -> music,
                                         connection -> StreamingConnection.closeConnection());
