...
public class c7_ErrorHandling extends ErrorHandlingBase {
    ...
    public void error_reporter() {
        Flux<String> messages = messageNode().onErrorResume(throwable -> errorReportService(throwable)
    ...
    public void unit_of_work() {
        Flux<Task> taskFlux = taskQueue().flatMap(task -> task.execute()
                                                              .onErrorResume(throwable -> task.rollback(throwable))
                                                              .then(task.commit())
                                                              .thenReturn(task));
    ...
    public void billion_dollar_mistake() {
        Flux<String> content = getFilesContent()
            ...
            .onErrorContinue((throwable, o) -> System.out.println(o));
    ...
    public void good_old_polling() {
        Flux<String> alerts = nodeAlerts().repeatWhenEmpty(alert -> alert.delayElements(Duration.ofSeconds(1)))
                                          .repeat();
